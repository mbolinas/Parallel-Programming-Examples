...
nsteps=%d, wstep=%d\n",
 nx, c, height_init, width_init, nsteps, wstep);
  ...
  printf("%d \n", nx);
  fflush(stdout);
  HANDLE_ERROR(cudaMalloc( (void**)&d_prev, (nx) * sizeof(double)));
  HANDLE_ERROR(cudaMalloc( (void**)&d_curr, (nx) * sizeof(double)));
  HANDLE_ERROR(cudaMalloc( (void**)&d_next, (nx) * sizeof(double)));
...
exp(-1.0/(1-SQR(2.0*(i-width_init/2.0)/width_init)));
  ...
  HANDLE_ERROR(cudaMemcpy( d_prev, u_prev, (nx) * sizeof(double), cudaMemcpyHostToDevice ));
  
  HANDLE_ERROR(cudaMemcpy( d_curr, u_curr, (nx) * sizeof(double), cudaMemcpyHostToDevice ));
  
  HANDLE_ERROR(cudaMemcpy( d_next, u_next, (nx) * sizeof(double), cudaMemcpyHostToDevice ));
...
__global__ void update(double *d_prev, double *d_curr, double *d_next, int nx, int k) {
  int i;
  //double *tmp;  
  ...
  //cyclic distribution
  for (i = tid; i < nx-1; i = i + span){
    if(i==0)continue;
    d_next[i] = 2.0*d_curr[i] - d_prev[i] + k*(d_curr[i+1] + d_curr[i-1] - 2.0*d_curr[i]);
  }
...
int main(int argc, char *argv[]) {
  ...
  for (iter = 1; iter <= nsteps; iter++) {
    update<<<nx,1>>>(d_prev, d_curr, d_next, nx, k);
    //cudaDeviceSynchronize();     

    HANDLE_ERROR( cudaMemcpy( u_curr, d_next, sizeof(double) * nx, cudaMemcpyDeviceToHost ));
    HANDLE_ERROR( cudaMemcpy( u_prev, d_curr, sizeof(double) * nx, cudaMemcpyDeviceToHost ));
    HANDLE_ERROR( cudaMemcpy( d_curr, u_curr, sizeof(double) * nx, cudaMemcpyHostToDevice ));
    HANDLE_ERROR( cudaMemcpy( d_prev, u_prev, sizeof(double) * nx, cudaMemcpyHostToDevice ));

    if (wstep!=0 && iter%wstep==0){
      //for(int i = 0; i < 20; i++){
	//printf("%.2f -> %.2f, ", u_prev[i], u_curr[i]);
      //}
      //printf("\n\n\n\n");
      //fflush(stdout);
